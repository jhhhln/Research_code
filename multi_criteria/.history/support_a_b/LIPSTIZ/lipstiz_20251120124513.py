...
太棒了，很高兴之前的修改方案奏效了！针对你新提供的代码（增加了参数 a 和 b），我们要应用同样的**“连续拒绝计数器”（Rejection Limit）**逻辑。原有的 LIPO 算法 1 依赖于拒绝采样：如果当前随机点 $s$ 的上界（Upper Bound）低于已知的最大值 $max\_g\_value$，就不计算它。随着优化进行，满足条件的点越来越难找，导致 while 循环空转。以下是修改后的完整代码。我对 optimize_s 做了防死循环处理，同时顺手修正了你 optimize_y 函数末尾的一个变量名小错误（max_g_value -> min_g_value）。修改后的代码Pythonimport numpy as np
import random
from scipy.optimize import minimize_scalar, minimize
from support_a_b.Concave.sdp_with_a_b import solve_sdp
...
def optimize_s(I, m, c, r, a, b, y, lambda_val, num_iterations):
    # 动态调整 Lipschitz 常数
    L = lambda_val * (r + c) if lambda_val > 1e-6 else 1.0
    ...
    max_rejections_allowed = 2000  # 允许连续失败的次数，超过则认为已收敛
