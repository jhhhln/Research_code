...
def run_parallel_experiment(mu_values, r, I, c, a, b, max_workers=4):
    results = []

    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(run_single_experiment, c, r, I, np.array([1, mu]), b): mu
            for mu in mu_values
        }

        for future in tqdm(as_completed(futures), total=len(futures), desc="计算进度"):
            mu = futures[future]
            try:
                res = future.result()
                results.append(res)
