#在低阶矩的情况下 直接利用s的凹性搜索最值
import numpy as np
from scipy.optimize import minimize_scalar, minimize
from sdp_wit import solve_sdp
from fractions import Fraction

def optimize_s(I, m, c, r, b, y, lambda_val):
    ...
    for i, (left, right) in enumerate(valid_intervals):
        ...
        endpoint_values = []
        for s in [left, right]:
            val = solve_sdp(I, m, c, r, b, y, s, lambda_val)
            endpoint_values.append((s, val))
        try:
            result = minimize_scalar(
                lambda s: -solve_sdp(I, m, c, r, b, y, s, lambda_val), 
                bounds=(left, right),
                method='bounded'
            )
            interior_s = result.x
...
def optimize_y(I, m, c, r, b, lambda_val):
    if lambda_val==0:
        s_opt =-0.1
        def objective_y(y):
        ...
        def objective(y):
            _, max_g = optimize_s(I, m, c, r, b, y, lambda_val)
            return max_g 
...
if __name__ == "__main__":
    ...
    s = 0.8
    lambda_val =0
    y_opt, g_opt = optimize_y(I, m, c, r, b, lambda_val)
