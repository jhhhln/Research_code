...
#support为[a,b]
def solve_sdp(I, m, c, r, b, y, s, lambda_val):
    # 预计算所有需要的组合数，是一个（I+1）*（I+1）的矩阵，范围是0到I
    comb_dict = {}
    max_deg = I + 1
    ...
    if lambda_val == 0:
        ...
        for l in range(I + 1):
            sum_val = 0
            for r_val in range(l + 1):
                if r_val <= I:
                    comb_val = comb_dict.get((I - r_val, l - r_val), 0)
                    sum_val += beta[r_val] * comb_val * y_powers[r_val]
        ...
        if s >= y:
            ...
            for l in range(I + 1):
                ...
                for r_val in range(l + 1):
                    if r_val <= I:
                        comb_val = comb_dict.get((I - r_val, l - r_val), 0)
                        sum_val += beta[r_val] * comb_val * s_powers[r_val]
                indices = [(i, j) for i in range(I + 1) for j in range(I + 1) if i + j == 2 * l]
                constraints.append(sum_val == cp.sum([x[i, j] for (i, j) in indices]))
            # z 约束
            for l in range(1, I + 1):
