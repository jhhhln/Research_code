...
def run_parallel_experiment_mub(mu_values, r, I, c, b, max_workers=None):
    """
    使用多进程并行执行 μ/b 实验
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(run_single_mub, mu, r, I, c, b): mu
            for mu in mu_values
        }
...
if __name__ == "__main__":
    ...
    b =5
    a=1
    mu_values =[0.5,1,1.5,2,2.5,3,3.5,4,4.5,5]

    df_mu = run_parallel_experiment_mub(mu_values, r, I, c, b, max_workers=6)

    df_mu.to_csv('mu_b_experiment.csv', index=False)
    ...
    df_mu.to_csv('mu_b_experiment_1.csv', index=False)
    print("✅ 实验完成，结果已保存为 multi_mu_b_experiment.csv")
    ...
    df_mu.to_csv('mu_b_experiment_2.csv', index=False)
    print("✅ 实验完成，结果已保存为 multi_mu_b_experiment.csv")
    ...
    df_mu.to_csv('mu_b_experiment_3.csv', index=False)
    print("✅ 实验完成，结果已保存为 multi_mu_b_experiment.csv")
    ...
    c = 0.1
    b = 1
    mu_values =np.arange(0.05, 0.95, 0.05)
    df_mu = run_parallel_experiment_mub(mu_values, r, I, c, b, max_workers=6)

    df_mu.to_csv('mu_b_experiment_4.csv', index=False)
    print("✅ 实验完成，结果已保存为 multi_mu_b_experiment.csv")