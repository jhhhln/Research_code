...
def find_feasible_bounds(I, m, c, r, b, b_val, Z1_star, y_opt_Z1):
    # 定义约束函数
    def constraint_func(y):
        return Z1_y(y, I, m, c, r, b) - Z1_star*b_val
    ...
    try:
        if constraint_func(0)>=0:
            ...
            y_min = binary_search_root(constraint_func, 0, y_opt_Z1)
    except ValueError:
    ...
    try:
        if constraint_func(b)>=0:
            ...
            y_max = binary_search_root(constraint_func, y_opt_Z1, b)
    except ValueError:
        y_max = b  # 当约束在右半区间始终满足时取最大值
    
    return y_min, y_max

def compute_optimal_y_and_value(I, m, c, r, b, b_val, Z3_star, Z1_star, y_opt_Z1):
    y_min, y_max = find_feasible_bounds(I, m, c, r, b, b_val, Z1_star, y_opt_Z1)
...
def concave_maximization_over_convex_set(I, m, c, r, b, b_val, Z3_star=None, Z1_star=None, y_opt_Z3=None, y_opt_Z1=None):
    ...
    # 检查Z3最优点是否满足约束
    Z1_at_Z3_opt = Z1_y(y_opt_Z3, I, m, c, r, b)
    Z1_normalized = Z1_at_Z3_opt/Z1_star 
    
    if Z1_normalized >= b_val:
        return y_opt_Z3, Z3_y(y_opt_Z3, I, m, c, r, b) / Z3_star
    
    # 寻找边界解
    return compute_optimal_y_and_value(I, m, c, r, b, b_val, Z3_star, Z1_star, y_opt_Z1)
...
def generate_pareto_frontier(I, m, c, r, b, num_points, filename):
    ...
    print(f"Z3* = {Z3_star:.4f}, Z3* = {Z1_star:.4f}")
    
    # 创建权衡参数数组 (从0到1)
    alpha_values = [i / 100 for i in range(2, 100, 2)]
    
    # 创建CSV文件并写入表头
    fieldnames = ['alpha', 'y_opt', 'Z3_actual', 'Z1_actual', 'normalized_Z3', 'Z3_ratio', 'Z1_ratio']
    ...
    for i, alpha in enumerate(alpha_values):
        try:
            ...
            y_opt, normalized_Z3 = concave_maximization_over_convex_set(
                I, m, c, r, b, b_val, Z3_star, Z1_star, y_opt_Z3, y_opt_Z1)
    ...
    # 提取帕累托点用于绘图
    pareto_points = np.array([(row['Z3_actual'], row['Z1_actual']) for _, row in df.iterrows()])
    
    return df, pareto_points, Z3_star, Z1_star
if __name__ == "__main__":
    ...
    # # 生成帕累托前沿并保存结果
    # results_df, pareto_points, Z3_star, Z2_star = generate_pareto_frontier(
    #     I, m, c, r, b, num_points=2, filename="pareto_results_I1.csv"
    # )
    results_df, pareto_points, Z3_star, Z1_star = generate_pareto_frontier(
        I, m, c, r, b, num_points=2, filename="pareto_results_Z3Z1_I1_0827.csv"
    )
    ...
    # # # 帕累托前沿
    # # plt.plot(Z3_values, Z3_values, 'b-', linewidth=2, label='Pareto Frontier')
    

    # # # 参考线
    # # plt.axhline(y=Z3_star, color='g', linestyle='--', alpha=0.3)
    ...
    # # plt.title('Pareto Frontier:Z3 vs Z3', fontsize=14)
    ...
    # 生成帕累托前沿并保存结果
    # results_df, pareto_points, Z3_star, Z2_star = generate_pareto_frontier(
    #     I, m, c, r, b, num_points=20, filename="pareto_results_I2_1.csv"
    # )
    results_df, pareto_points, Z3_star, Z1_star = generate_pareto_frontier(
        I, m, c, r, b, num_points=20, filename="pareto_results_Z3Z1_I2_1_0827.csv"
    )
